...
<div class="container mx-auto px-4 max-w-7xl">
    ...
    <div class="bg-white shadow-lg rounded-lg p-8">
        <form action="/invoices/store" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                <div>
                    ...
                    <select id="customerdropdown" name="customer" class="w-full border border-gray-400 rounded-lg px-4 py-3 focus:ring-blue-500 focus:border-blue-500" required>
                        ...
                        <option value="{{ $customer->id }}">Name: {{ $customer->name }} | Phone: {{ $customer->phone }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    <label for="date" class="block text-gray-800 font-semibold mb-2">Date</label>
                    <input type="date" id="date" name="date" class="w-full border border-gray-400 rounded-lg px-4 py-3 focus:ring-blue-500 focus:border-blue-500" required>
                </div>
                <div>
                    <label for="reference" class="block text-gray-800 font-semibold mb-2">Reference (Optional)</label>
                    <input type="text" id="reference" name="reference" class="w-full border border-gray-400 rounded-lg px-4 py-3 focus:ring-blue-500 focus:border-blue-500">
                </div>
                <div>
                    <label for="vehicle_number" class="block text-gray-800 font-semibold mb-2">Vehicle Number (Optional)</label>
                    <input type="text" id="vehicle_number" name="vehicle_number" class="w-full border border-gray-400 rounded-lg px-4 py-3 focus:ring-blue-500 focus:border-blue-500">
                </div>
            </div>
            ...
            <button type="button" onclick="addNewLine()" class="mt-4 bg-green-500 text-white py-3 px-8 rounded shadow-md hover:bg-green-600">Add New Line</button>
            ...
            <div class="mt-8 bg-gray-50 p-6 rounded-lg shadow">
                <div class="flex justify-between items-center font-medium text-lg">
                    ...
                    <input type="number" id="product-total-for-hidden" name="product-total-for-hidden" class="w-32 px-4 py-3 border border-gray-400 rounded-lg focus:ring-blue-500 focus:border-blue-500 text-blue-600" placeholder="Product Total" min="0" hidden>
                </div>
                <div class="flex justify-between items-center mb-4">
                    <label for="freight" class="text-gray-800 font-medium">Freight:</label>
                    <input type="number" id="freight" name="freight" class="w-32 px-4 py-3 border border-gray-400 rounded-lg focus:ring-blue-500 focus:border-blue-500 text-blue-600" placeholder="Freight" min="0" onchange="updateTotals()">
                </div>
                <div class="flex justify-between items-center font-medium text-lg">
                    <span> Total of Product & Freight:</span>
                    <input type="number" id="total-of-product-and-freight-for-hidden" name="total-of-product-and-freight-for-hidden" class="w-32 px-4 py-3 border border-gray-400 rounded-lg focus:ring-blue-500 focus:border-blue-500 text-blue-600" placeholder="Total of Product & Freight" min="0" hidden>
                    <span id="total-of-product-and-freight">₹0.00</span>
                </div>
                <div class="flex justify-between items-center mb-4">
                    <label for="credit" class="text-gray-800 font-medium">Credit (Received):</label>
                    <input type="number" id="credit" name="credit" class="w-32 px-4 py-3 border border-gray-400 rounded-lg focus:ring-blue-500 focus:border-blue-500 text-red-700" placeholder="Credit" min="0" onchange="updateTotals()">
                </div>
                <div class="flex justify-between items-center font-medium text-lg">
                    <span>Grand Total (After Credit):</span>
                    <span id="grand-total">₹0.00</span>
                    <input type="number" id="grand-total-for-hidden" name="grand-total-for-hidden" class="w-32 px-4 py-3 border border-gray-400 rounded-lg focus:ring-blue-500 focus:border-blue-500 text-blue-600" placeholder="Grand Total" min="0" hidden>
                </div>
            </div>

            <button type="submit" class="mt-8 w-full bg-blue-500 text-white py-3 rounded shadow-md hover:bg-blue-600">Save</button>
        </form>
    </div>
</div>
@endsection
...
function addNewLine() {
    ...
    row.innerHTML = `
        ...
        <td class="px-6 py-3 border w-1/6">
            <input type="number" name="unit_price[]" class="w-full px-4 py-3 border border-gray-400 rounded-lg focus:ring-blue-500 focus:border-blue-500" placeholder="0" onchange="updateTotals()">
        </td>
        <td class="px-6 py-3 border w-1/6 text-right">
            <span class="item-total">₹0.00</span>
            <input type="number" name="total[]" class="w-full px-4 py-3 border border-gray-400 rounded-lg focus:ring-blue-500 focus:border-blue-500" placeholder="0" hidden>
        </td>
